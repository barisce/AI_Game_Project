...
public class EntityContainer {
	...
	public void Create(int x, int y, int entity, int owner){
		if(entity==1){
			CreateTown(x,y,owner);
		}else if(entity==2){
			CreateTower(x,y,owner);
		}else if(entity==3){
			CreateMine1(x,y,owner);
		}else if(entity == 4){
			CreateMine2(x,y,owner);
		}else if(entity == 5){
			CreateBarrack(x,y,owner);
		}else if(entity == 6){
			CreateTileImprovement(x,y,owner);
		}else if(entity == 7){
			CreateMelee(x,y,owner);
		}else if(entity == 8){
			CreateRange(x,y,owner);
		}else if(entity == 9){
			CreateCavalier(x,y,owner);
		}else if(entity == 10){
			CreateSiege(x,y,owner);
		}else if(entity == 11){
			CreateWorker(x,y,owner);
		}
	}
	...
	public void PrintToConsole(){
		...
		for(int i = 0;i<size;i++){
			temp = entities.get(i);
			System.out.println("Entity is:"+temp.getName()+" x and y:"+temp.getX()+" "+temp.getY()+" owner is: "+temp.getOwner());
		}
	}
